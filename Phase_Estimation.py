...
class Phase_Estimation:
    def __init__(self,
        ...
        self.signal_last_period  = np.zeros((self.max_length_vec_period, 6))
    ...
    def set_position(self, position, curr_time) -> float:  # TODO change name of the function?
        ...
        if not self.is_first_period_estimated:
            if self.time_vec[-1] > self.wait_time + self.listening_time:
                self.signal_last_period = compute_signal_period_autocorrelation(
                    x_vec = self.x_vec,
                    y_vec = self.y_vec,
                    z_vec = self.z_vec,
                ...
                self.look_ahead_range = max(1, int(len(self.signal_last_period) * self.look_ahead_pcent / 100))   # range_post is the number of points after the last nearest point on which estimate the new phase
                self.look_behind_range  = max(1, int(len(self.signal_last_period) * self.look_behind_pcent / 100))
                if self.reference is not None:  self.compute_phase_offset()
                for i in range(len(self.signal_last_period), len(self.x_vec) - 1):
                    p = np.array([self.x_vec[i], self.y_vec[i], self.z_vec[i], self.vel_x_vec[i], self.vel_y_vec[i], self.vel_z_vec[i]])
                    self.compute_phase(p)
                    self.estimate_period(p)
        ...
        self.compute_phase(p) # compute the phase

        if self.is_first_period_estimated:  self.estimate_period(p)

        return np.mod(self.phase[1]+self.offset, 2*np.pi)


    def estimate_period(self, p): # receives position and velocity as input and updates the vector of the last complete period when the phase completes a full cycle.
        if self.phase[1] - self.phase[0] < -self.epsilon:
            if abs(self.counter-len(self.signal_last_period)) < len(self.signal_last_period)*self.diff_len_new_ref/100: # Ensure that the difference in length between the new period and the previous one is not greater than the range set by the user.
                self.signal_last_period = self.new_period[0:self.counter, :]
                self.look_ahead_range = int(len(self.signal_last_period) * self.look_ahead_pcent / 100)
                if self.look_ahead_range==0:
                    self.look_ahead_range=1
                self.look_behind_range = int(len(self.signal_last_period) * self.look_behind_pcent / 100)
    ...
    def compute_phase(self, p):
        if self.is_first_period_estimated:
            length_reference = len(self.signal_last_period)
            if self.min_index_pre - self.look_behind_range < 1:
                ref_part1 = self.signal_last_period[0:self.min_index_pre + self.look_ahead_range]
                ref_part2 = self.signal_last_period[length_reference - self.look_behind_range + self.min_index_pre:length_reference]
                ...
                ref_part1 = self.signal_last_period[self.min_index_pre - self.look_behind_range:length_reference]
                ref_part2 = self.signal_last_period[0:self.min_index_pre + self.look_ahead_range - length_reference]
                ref = np.vstack((ref_part1, ref_part2))
                ...
                ref = self.signal_last_period[self.min_index_pre - self.look_behind_range:self.min_index_pre + self.look_ahead_range]
    ...
    def compute_phase_offset(self) -> None:
        ...
        rotated_ref = np.dot(self.signal_last_period[:, 0:3], rotation_matrix.T)
